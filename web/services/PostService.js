...
const generateStageQueryAllActiveSales = (loc, maxDistance, query) => {
	const stages = [];
	...
	if (query.formality) {
		matchStage.formality = parseInt(query.formality, 0);
	}

	if (query.type) {
		matchStage.type = parseInt(query.type, 0);
	}

	if (query.price) {
		if (query.price === '-1' || query.price === -1) {
			matchStage['$or'] = [
				{price: -1},
				{price: null}
			];
		} else {
			matchStage.price = parseInt(query.price, 0);
		}
	}
